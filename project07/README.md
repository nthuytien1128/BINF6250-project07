...
## Fardina Tabassum
This project was a bit challenging at first especially understanding the suffix array. While I found this project more manageable in terms of data scale than previous assignments, the conceptual complexity required a higher level of precision in my Python scripting. I spent a considerable amount of time debugging the occur and count tables, realizing that a single indexing error could collapse the entire backward search range.
I initially found the abstract nature of the FM index specifically the last-to-first mapping to be quite a hurdle to visualize. Overall I think implementing the Burrows-Wheeler Transform was a significant turning point in my understanding of how massive genomic datasets are actually handled in the field. Implementing run-length encoding on the BWT string also showed me how we can achieve high density data compression without losing the ability to perform exact string matches.
